...
import Crypto.Hash (Digest, HashAlgorithm, SHA256, hash)
import Data.Bits (
    countLeadingZeros,
    finiteBitSize,
    testBit,
...
verifyInclusionProof
    ...
    -- ^ The target hash (leaf digest)
    -> Digest ha
    -- ^ Merkle Tree Hash (root digest) for the tree size
    -> InclusionProof ha
    -- ^ InclusionProof of the target
    -> Bool
...
generateConsistencyProof
    :: TreeSize -> TreeSize -> MerkleHashTrees inp ha -> Maybe (ConsistencyProof ha)
